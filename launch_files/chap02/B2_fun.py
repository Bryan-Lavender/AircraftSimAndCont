...
from viewers.b2_viewer import B2Viewer
...
B2_view = B2Viewer(app=app)  # initialize the B2 viewer
...
while sim_time < end_time:
    ...
    if motions_time < time_per_motion:
        ...
        state.north += 30*SIM.ts_simulation
        state.psi += 8*SIM.ts_simulation
    elif motions_time < time_per_motion*5:
        state.north += 30*SIM.ts_simulation
        state.theta += 0.7*SIM.ts_simulation
        state.psi += 2*SIM.ts_simulation
    else:
        state.phi += 0.8*SIM.ts_simulation
        state.psi +=2*SIM.ts_simulation
    # -------update viewer and video-------------
    B2_view.update(state)
    B2_view.process_app()
