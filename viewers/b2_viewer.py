#Copy and paste B2_viewer.py here, and then edit to make it B2 specific.
"""
scsim_python: B2 viewer (for appendix C)
...
from viewers.draw_b2 import DrawB2
...
class B2Viewer():
    def __init__(self, app):
        ...
        self.window.setWindowTitle('B2 Viewer')
        ...
        self.plot_initialized = False # has the B2 been plotted yet?
        self.sc_plot = []

    def update(self, state):
        # initialize the drawing the first time update() is called
        if not self.plot_initialized:
            self.sc_plot = DrawB2(state, self.window)
            self.plot_initialized = True
        # else update drawing on all other calls to update()
        else:
            self.sc_plot.update(state)
        # update the center of the camera view to the B2 location
